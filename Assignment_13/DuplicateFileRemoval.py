...
class DFR:
    ...
    def hashfile(self,blocksize=1024):
        path = self.path
        fd = open(path, 'rb')
        hasher = hashlib.md5()
    ...
    def DeleteFiles(self,dict1,log_dir):
        dict1 = self.dups
...
def main():
    ...
    try:
        obj = DFR()
        arr = {}
        arr = obj.FindDuplicate(argv[1])
        obj.DeleteFiles(arr,argv[1])
        schedule.every(int(argv[2])).minutes.do(obj.SendMail,argv[3])
